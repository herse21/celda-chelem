...
La aplicación es una experiencia cartográfica de una sola página. `app/page.tsx` abre el explorador; `components/territory-map.tsx` contiene la integración MapLibre; las fichas, fuentes y el recorrido viven en componentes separados. Carga derivados GeoJSON locales para mantener procedencia y solicita Esri World Imagery como fondo visual cuando el usuario entra al mapa.
