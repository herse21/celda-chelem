...
import { geometryCenter, SatelliteMedia } from './satellite-media';
export function FeatureDetails({ feature, source, onEvidence }: { feature: TerritoryFeature; source?: Source; onEvidence: (id: string) => void }) {
  ...
  const center=geometryCenter(feature.geometry as {coordinates?:unknown});
  return <>{center && <SatelliteMedia coordinates={center} title={locality && typeof p.name==='string'?p.name:themes.find(t=>t.id===p.theme)?.title} compact />}<div className="badge-row"><span className="badge official" title="Dato publicado por una institución pública">ORIGEN OFICIAL</span><span className="badge derived" title="Información preparada a partir de fuentes oficiales. No corresponde a un dato publicado directamente por la institución.">DERIVADO</span></div>
