...
import { ArrowDownRight, ArrowUpRight, ArrowLeft, ArrowRight, BookOpen, Compass, Layers2, Minus, Plus, Waves, X, LocateFixed, Info, Route, MapPin } from 'lucide-react';
...
export default function Explorer() {
  ...
  function guide(index: number) {
    ...
    setOpening(null); setEntered(true); setChapter(index); setLayersOpen(false); setSelected(null); setPanel(null);
  ...
  function selectPlace(id: string) {
    ...
    setPanel(null); setEntered(true); setOpening(null); setLayersOpen(false);
  ...
  return <main className={`explorer ${entered ? 'is-entered' : 'is-intro'}`}>
    {config && data && <TerritoryMap ref={map} config={config} data={data} visibility={visibility} entered={entered} selected={selected} onSelect={setSelected} onReady={() => setReady(true)} onError={setError} />}
    ...
    <footer className="map-footer"><div className="credit"><b>EVEN</b><span>×</span><span>Colectivo por la Costa</span></div><div className="map-attribution">Datos: INEGI · CONABIO <span> / </span> Preparación: EVEN</div></footer>
    <Sheet open={!!panel} onOpenChange={open => { if (!open) setPanel(null); }}><SheetContent className="information-sheet"><SheetHeader><p className="eyebrow">CELDA CHELEM</p><SheetTitle>{titles[panel ?? 'sources']}</SheetTitle><SheetDescription>{descriptions[panel ?? 'sources']}</SheetDescription></SheetHeader><div className="sheet-scroll">{panel === 'sources' ? <EvidencePanel sources={sources} focus={sourceFocus} /> : panel === 'places' ? <div className="place-list">{config?.places.map(p => <button key={p.id} onClick={() => selectPlace(p.id)}><span><small>LOCALIDAD · INEGI</small>{p.name}</span><ArrowUpRight /></button>)}{!config && <p>Las localidades aún no están disponibles. Vuelve a cargar la página.</p>}</div> : <AboutPanel />}</div></SheetContent></Sheet>
